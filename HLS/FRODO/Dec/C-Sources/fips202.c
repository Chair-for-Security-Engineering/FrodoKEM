...
/*
    Absorbs a the input and generates the output if needed. Can keep its state and 
    absorb further inputs if needed
    Inputs:  uint16_t *input    : input array
             uint16_t outlen    : length of the output 
             uint16_t inlen     : input length (in full 64-bit words)
             uint8_t reset      : indicates if the state should be zeroed at the start
    ...
    uint16_t inlen, uint8_t reset, uint8_t begin)
{
    ...
    if(begin)
    {
        ...
        Absorb: while(inlen > 0)
        {
            ...
            sum = inlen + start;
            //end indicates how many 64 bit words are absorbed. Either 21 if remaining
            //inlen is larger than SHAKE128_RATE, or less if not.
            //As absorbing can resume in the middle of a block, start has to be
            ...
            //Permute the state, but only if the block is full AND more inputs will
            //be absorbed (if no more inputs, the permutation happens in the squeezing phase)
        ...
        if(outlen > 0)
        {
            //Finish padding and permute again
...
/*
    Generate S and E, both in a certain range indicates by low1/2 and high1/2
    Inputs :  uint16_t outlen   : total number of samples for S and E
              uint16_t *input   : seedSE that is hashed
              uint16_t low1     : lower bound for S
              uint16_t high1    : upper bound for S
              uint16_t low2     : lower bound for E
              uint16_t high2    : upper bound for E
              uint8_t begin     : decides if the function is executed
    Outputs : uint16_t *S       : S
              uint16_t *E       : E
*/
    ...
    uint16_t low1, uint16_t high1, uint16_t low2, uint16_t high2, uint8_t begin)
{
    ...
    if(begin)
    {
        ...
        //Absorb the 9 input bytes and apply the padding
        ...
        //Produce the output. Keep track of how many 64 bit words have been generated
        //(index) and how many have been written into S (index_s) and E (index_e)
        //Writing only happens if index is between low1 and high1 (write into S) or low2
        //and high2 (write into E)
